    ...
    namespace db{
        class DbConnection
        {
            public:
                ...
                sql::ResultSet* executeQuery(const std::string& sqlquery, Args&&... args)
                {
                    std::lock_guard<std::mutex> lock(mutex_);
                    try
                    {
                        std::unique_ptr<sql::PreparedStatement> stmt(conn_->prepareStatement(sqlquery));
                        bindParams(stmt.get(), 1, std::forward<Args>(args)...);
                        return stmt->executeQuery();
                    }
                    catch (const sql::SQLException& e) 
                    {
                        LOG_ERROR << "Query failed: " << e.what() << ", SQL: " << sqlquery;
                        throw DbException(e.what());
                    }
                }

                template<typename... Args>
                int executeUpdate(const std::string& sqlquery, Args&&... args)
                {
                    std::lock_guard<std::mutex> lock(mutex_);
                    try
                    {
                    ...
                    {
                        LOG_ERROR << "Update failed: " << e.what() << ", SQL: " << sqlquery;
                        throw DbException(e.what());
                    }
                }
